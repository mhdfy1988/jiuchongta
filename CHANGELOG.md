...
- 礼券牌系统（第 3 类消耗品，共 10 张）：立即生效型（下次出牌加成）与选牌型两类
- 塔罗牌扩充至 16 张（新增隐者/命运之轮/星星/月亮/太阳/力量/恶魔/倒吊人）
- 小丑牌扩充至 55 张（新增忠诚小丑/古怪小丑/靶心/重力/流浪者/暴怒/杂技/红丝绒/远古人/星座/帝皇/手相）
- 成就扩充至 20 个（新增首次消费/礼券初体验/满载而归/全牌型图鉴/完美一层/百万富翁/无丑通关/Boss杀手/弃牌大师/刷新达人）
- Boss 减益扩充至 22 个（新增矮墙/贬值/沉底/牙缝/倒钩/禁耗/立柱/燧石/吸血鬼/赌徒）
- 卡牌印记系统：金色印记（计分时 +3 倍率）、红色印记（出牌后重抽 1 张）
- 全局统计面板（总览 / 成就进度 / 解锁内容 / 游戏行为 / 牌型图鉴，宽体双列布局）
- 调试控制台（DebugConsole）：查看/修改游戏状态、发放卡牌、切换 Boss
- 手牌"清空选择"按钮
- 卡牌图鉴新增礼券标签页；Boss 页展示图标与动态详情

### 修复

- 幻视（Pareidolia）此前未实现：统一 isFace 判定，6 处人头牌检查全部生效（含与封王/石头小丑的反向联动）
- 补票改为永久被动小丑（出牌/换牌次数 +1），对齐 Balatro 原版设计
- 存档 Bug：失败后仍能继续游戏——防抖存档定时器在 clearSave/saveGameNow 时未清除（含第 9 层特定场景）
- 命运之轮使用后现在会提示具体升级了哪种牌型
- 燧石 Boss 的预览分未减半（预览与实际结算不一致）
- 牙缝/倒钩/吸血鬼 Boss 缺少反馈提示；立柱 Boss 侧栏现在列出失效牌
- 帝皇小丑在弱牌型上可能算出负分
- 完美一层成就在补票增加换牌次数后判定错误（改用"本层是否弃过牌"判定）
- 全局统计的总游戏次数从未写入（恒为 0）
- 商店卖出确认时悬浮提示遮挡取消按钮

### 优化

- 成就系统分页展示（每页 9 个，末页保持等高）
- 游戏结束弹窗、全局统计弹窗改为宽体双列布局
- 商店消耗品池配比：塔罗 45% / 星球 35% / 礼券 20%
- 测试总数 227（Boss/存档/计分覆盖增强）
