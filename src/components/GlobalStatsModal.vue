<template>
  <BaseModal size="sm" @close="$emit('close')">
    ...
    <div class="stats-section">
      ...
      <div v-if="handTypeList.length === 0" class="empty-text">尚未打出任何牌型</div>
      <div v-for="item in handTypeList" :key="item.name" class="stat-row">
        <span class="sr-label">{{ item.name }}</span>
        <span class="sr-value" :class="{ unlocked: item.unlocked }">{{ item.unlocked ? '✓' : '—' }}</span>
      </div>
    </div>
...
.modal-title { font-size: 18px; color: var(--gold); text-align: center; margin-bottom: 12px; }
...
.empty-text { font-size: 12px; color: var(--muted); text-align: center; padding: 8px; }
</style>
